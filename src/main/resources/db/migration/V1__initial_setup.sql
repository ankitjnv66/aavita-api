-- V202603040522__Initial_setup.sql
-- Idempotent + safe for repeated execution

-- USERS
CREATE TABLE IF NOT EXISTS users (
    id BIGSERIAL PRIMARY KEY,
    first_name VARCHAR(150) NOT NULL,
    last_name VARCHAR(150) NOT NULL,
    email VARCHAR(150) NOT NULL,
    phone_number TEXT NOT NULL,
    password TEXT NOT NULL,
    type TEXT NOT NULL
);

-- Ensure unique constraint separately (safer for idempotency)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'uk_users_email'
    ) THEN
        ALTER TABLE users ADD CONSTRAINT uk_users_email UNIQUE (email);
    END IF;
END $$;


-- SITES
CREATE TABLE IF NOT EXISTS sites (
    site_id UUID PRIMARY KEY,
    user_id BIGINT NOT NULL,
    username VARCHAR(150) NOT NULL,
    location VARCHAR(250) NOT NULL,
    created_on TIMESTAMPTZ NOT NULL
);

-- FK safe add
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sites_user'
    ) THEN
        ALTER TABLE sites
        ADD CONSTRAINT fk_sites_user
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS ix_sites_user_id ON sites(user_id);


-- DEVICES
CREATE TABLE IF NOT EXISTS devices (
    id BIGSERIAL PRIMARY KEY,
    site_id UUID NOT NULL,
    mesh_id VARCHAR(100) NOT NULL,
    src_mac VARCHAR(50) NOT NULL,
    dst_mac VARCHAR(50) NOT NULL,
    gateway_mac VARCHAR(50) NOT NULL,
    sub_gateway_mac VARCHAR(50) NOT NULL,
    pkt_id INTEGER NOT NULL,
    board_type SMALLINT NOT NULL,
    device_type SMALLINT NOT NULL,
    device_role SMALLINT NOT NULL,
    last_action_cause SMALLINT,
    last_pkt_type INTEGER,
    last_crc16 INTEGER,
    last_seen TIMESTAMPTZ NOT NULL,
    created_on TIMESTAMPTZ NOT NULL,
    updated_on TIMESTAMPTZ NOT NULL,
    user_id BIGINT NOT NULL
);

-- FKs separately (important for re-run safety)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_devices_site'
    ) THEN
        ALTER TABLE devices
        ADD CONSTRAINT fk_devices_site
        FOREIGN KEY (site_id) REFERENCES sites(site_id) ON DELETE CASCADE;
    END IF;
END $$;

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_devices_user'
    ) THEN
        ALTER TABLE devices
        ADD CONSTRAINT fk_devices_user
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS ix_devices_mesh_id ON devices(mesh_id);
CREATE UNIQUE INDEX IF NOT EXISTS ix_devices_site_id_src_mac ON devices(site_id, src_mac);
CREATE INDEX IF NOT EXISTS ix_devices_src_mac ON devices(src_mac);
CREATE INDEX IF NOT EXISTS ix_devices_user_id ON devices(user_id);


-- DEVICE COMMANDS
CREATE TABLE IF NOT EXISTS device_commands (
    id BIGSERIAL PRIMARY KEY,
    device_id BIGINT NOT NULL,
    pkt_type INTEGER NOT NULL,
    action_cause SMALLINT NOT NULL,
    serialized_payload TEXT NOT NULL,
    json_payload JSONB,
    status VARCHAR(50) NOT NULL,
    created_on TIMESTAMPTZ NOT NULL,
    executed_on TIMESTAMPTZ
);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_device_commands_device'
    ) THEN
        ALTER TABLE device_commands
        ADD CONSTRAINT fk_device_commands_device
        FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS ix_device_commands_device_id ON device_commands(device_id);


-- DIGITAL PINS
CREATE TABLE IF NOT EXISTS device_digital_pins (
    id BIGSERIAL PRIMARY KEY,
    device_id BIGINT NOT NULL,
    pin_number SMALLINT NOT NULL,
    state SMALLINT NOT NULL,
    updated_on TIMESTAMPTZ NOT NULL,
    UNIQUE(device_id, pin_number)
);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_digital_pins_device'
    ) THEN
        ALTER TABLE device_digital_pins
        ADD CONSTRAINT fk_digital_pins_device
        FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE;
    END IF;
END $$;


-- PWM PINS
CREATE TABLE IF NOT EXISTS device_pwm_pins (
    id BIGSERIAL PRIMARY KEY,
    device_id BIGINT NOT NULL,
    pin_number SMALLINT NOT NULL,
    value SMALLINT NOT NULL,
    updated_on TIMESTAMPTZ NOT NULL,
    UNIQUE(device_id, pin_number)
);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_pwm_pins_device'
    ) THEN
        ALTER TABLE device_pwm_pins
        ADD CONSTRAINT fk_pwm_pins_device
        FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE;
    END IF;
END $$;


-- STATUS HISTORY
CREATE TABLE IF NOT EXISTS device_status_history (
    id BIGSERIAL PRIMARY KEY,
    device_id BIGINT NOT NULL,
    pkt_type INTEGER NOT NULL,
    action_cause SMALLINT NOT NULL,
    serialized_payload TEXT NOT NULL,
    json_payload JSONB,
    received_on TIMESTAMPTZ NOT NULL
);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_status_history_device'
    ) THEN
        ALTER TABLE device_status_history
        ADD CONSTRAINT fk_status_history_device
        FOREIGN KEY (device_id) REFERENCES devices(id) ON DELETE CASCADE;
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS ix_device_status_history_device_id ON device_status_history(device_id);

CREATE INDEX IF NOT EXISTS ix_users_email ON users(email);